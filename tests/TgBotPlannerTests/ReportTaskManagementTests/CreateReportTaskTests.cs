...
namespace TgBotPlannerTests.ReportTaskManagementTests;
...
public sealed class CreateReportTaskTests
{
    ...
    public async Task Create_ReportTask_With_Relative_DateTime_Yesterday()
    {
        ...
        Random random = new();
        CreateReportTaskCommand command = new(input, random.Next(1, 1000));
        try
        {
    ...
    public async Task Create_ReportTask_MultiThread()
    {
        ...
        IServiceProvider provider = scope.ServiceProvider;
        Random random = new();
        CreateReportTaskCommand command1 = new(input, random.Next(1, 1000));
        CreateReportTaskCommand command2 = new(input, random.Next(1, 1000));
        try
        {
