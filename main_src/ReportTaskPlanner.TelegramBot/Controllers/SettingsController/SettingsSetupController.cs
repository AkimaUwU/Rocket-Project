...
[BotHandler]
public sealed record SettingsSetupController
{
    ...
    public SettingsSetupController(TimeZoneDbOptionsFileRepository repository, ILogger logger) => 
        (_repository, _logger) = (repository, logger);
    
    [ReplyMenuHandler(CommandComparison.Equals, StringComparison.OrdinalIgnoreCase, ["/start", "/update_time_api_key"])]
    public async Task OnStart(ITelegramBotClient client, Update update)
    {
        ...
        {
            Result<TimeZoneDbOptions> timeZoneOptions = _repository.Get(ConfigurationVariables.TimeZoneDbTokenConfigPath);
            if (timeZoneOptions.IsSuccess)
                return;
        }
        ...
        SettingsSetupCache cache = new SettingsSetupCache();
        long userId = update.Message!.From!.Id;
        cache.RegisterStep(userId);
        cache.UpdateStep(userId, SettingsSetupCache.start);
        cache.AddMessage(await Message.Send(client, update, replyMessage, options));
        
        SettingsSetupCommandStep step = new SettingsSetupCommandStep(OnContinue, cache);
        update.RegisterStepHandler(step);
        _logger.Information("{Context}. Time Zone Db Api Token Configuration started.", nameof(SettingsSetupController));
    }
    ...
    public async Task OnContinue(ITelegramBotClient client, Update update)
    {
        ...
        SettingsSetupCommandStep? previous = update.GetStepHandler<SettingsSetupCommandStep>();
        if (previous == null)
            return;
        ...
        previous.Cache.AddMessage(await Message.Send(client, update, "Продолжаем настройку", options));
        ...
        previous.Cache.UpdateStep(userId, SettingsSetupCache.waitsForToken);
        
        SettingsSetupCommandStep next = previous.CreateContinueStep(OnClientTokenReply);
    ...
    public async Task OnClientTokenReply(ITelegramBotClient client, Update update)
    {
        ...
        foreach (var character in text)
        {
         ...
         Result saving = _repository.Save(ConfigurationVariables.TimeZoneDbTokenConfigPath, text);
         if (!saving.IsSuccess)
         {
             string errorMessage = $"""
        ...
        _logger.Information("{Context}. Time Zone Db Api Token Configuration saved.", nameof(SettingsSetupController));
        await client.RegisterCommandIfNotExists("/update_time_api_key", "Опция обновления Time Zone Db ключа");
        await client.RegisterCommandIfNotExists("/has_time_api_key", "Опция проверки Time Zone Db ключа");
    }
    
    [ReplyMenuHandler(CommandComparison.Equals, StringComparison.OrdinalIgnoreCase, ["/stop_config"])]
    public async Task OnCancel(ITelegramBotClient client, Update update)
    {
        ...
        _logger.Information("{Context}. Time Zone Db Api Token Configuration canceled.", nameof(SettingsSetupController));
    }

    [ReplyMenuHandler(CommandComparison.Equals, StringComparison.OrdinalIgnoreCase, ["/has_time_api_key"])]
    public async Task HasApiToken(ITelegramBotClient client, Update update)
    {
        await update.RemoveLastMessage(client);
        
        Result<TimeZoneDbOptions> timeOptionsResult = _repository.Get(ConfigurationVariables.TimeZoneDbTokenConfigPath);
        var message = timeOptionsResult.IsSuccess switch
        {
            true => await Message.Send(client, update, "В боте существуте конфигурация Time Zone Db."),
            false => await Message.Send(client, update, "В боте отсутствует конфигурация Time Zone Db.")
        };
    }
}