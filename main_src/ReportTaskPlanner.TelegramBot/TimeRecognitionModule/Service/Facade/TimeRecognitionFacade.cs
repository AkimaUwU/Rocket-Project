...
using ReportTaskPlanner.TelegramBot.TimeRecognitionModule.Models.RecognitionStrategies;
...
public class TimeRecognitionFacade : ITimeRecognitionFacade
{
    ...
    )
    {
        ...
        if (calculated.TimeStamp < time.TimeStamp)
            return new Error("Нельзя создать задачу прошедшего времени.");
        ReportTaskSchedule schedule = new(time.TimeStamp, calculated.TimeStamp);
        schedule = ticket.ApplyModification(schedule);
        return schedule;
    }
